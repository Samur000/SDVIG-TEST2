...
export function FinancePage() {
  ...
  const getTxCurrency = (walletId: string) => state.wallets.find(w => w.id === walletId)?.currency ?? 'RUB';

  // Аналитика за месяц (перевод: отток = расход, приток = доход)
  const monthByCurrency = useMemo(() => {
    const income: Record<string, number> = {};
    const expense: Record<string, number> = {};
    state.transactions.filter(t => isThisMonth(t.date)).forEach(t => {
    ...
    all.forEach(cur => { net[cur] = (income[cur] ?? 0) - (expense[cur] ?? 0); });
    return { income, expense, net, allCurrencies: all };
  }, [state.transactions, state.wallets]);

  const monthIncome = Object.values(monthByCurrency.income).reduce((a, b) => a + b, 0);
  const monthExpenses = Object.values(monthByCurrency.expense).reduce((a, b) => a + b, 0);
